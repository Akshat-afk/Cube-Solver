...
class CubeStatus:
    ...
    SIDE_AREA_SIZE = 50
    SIDE_AREA_GAP = 5
    SIDE_AREA_POSITION = {'U': (1, 0), 'R': (2, 1), 'F': (
        1, 1), 'D': (1, 2), 'L': (0, 1), 'B': (3, 1)}
        ...
        for idx, side in enumerate(self.side_order):
            ...
            for face in faces:
                ax.add_collection3d(Poly3DCollection(
                    [face], facecolors=color, linewidths=1, edgecolors='r', alpha=0.8))
    ...
    def color_name_to_rgba(self, color_name):
        ...
        return color_dict.get(color_name, (1.0, 1.0, 1.0, 1.0))
